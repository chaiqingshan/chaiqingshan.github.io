...
var total_his;
//加载全部报表信息 
function getAllNewsInfo(token, page_num, flag){
	$.ajax({
        type: "post",
        url: BASE_URL + '/rep/loadAllCaseReport',
        data: JSON.stringify({
            "params": {
                "token": token,
                "page_num": page_num,
        ...
        success: function(res) {
            ...
            total_his = res.detail.pageResult.total;
            html = '';
            for(var i=0;i<reportList.length;i++){
                ...
                html  += '<li class="list-group-item" id="'+listItem+'" reportId="'+reportId+'">'
                            + '<input type="checkbox" style="width: 12px;height: 12px;float:left;margin-left:-5px;" class="single_check check">'
                            ...
                            + '<span style="margin-left:-20px;display:block;width:200px;float:left;">'
                                + '第' + week + '周警情周报'
                            + '</span>'
                            + '<span style="display:block;width:200px;margin-left:400px;float:left;">'
                                + rightTime
                            + '</span>'
                            + '<span class="pull-right hide" style="float:left;" id="'+spanItem+'">'
                                + '<i class="iconfont" style="margin-right:10px;color:#999999;">&#xe606;</i>'
                                + '<i class="iconfont" style="color:#999999;">&#xe602;</i>'
                            + '</span>'
                        + '</li>';
                if(i == 3){
                    html += '<div class="month_paper"><label>更早（25件）</label></div>'; 
                }
            } 
            $("#div").append(html);  
        },
...
function pageonLoad(){
    flag = 2;
    getAllNewsInfo(token, page, flag);
    $("#News-Pagination").pagination(total_his, {
